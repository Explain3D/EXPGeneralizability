...
from captum.attr import IntegratedGradients, NoiseTunnel, Saliency, GuidedBackprop, InputXGradient, DeepLift, LRP
...
def IG_purify_top_k(data, model, gt_cls, ori_pred, n_csder=3, IG_step=25, smooth=True):
    explain = InputXGradient(model)
    gt_cls = int(gt_cls)
    
    if smooth == True:
        nt = NoiseTunnel(explain)
        gt_pos_mask = nt.attribute(data, nt_type='smoothgrad', nt_samples=10, target=gt_cls)
    
    elif smooth == False:
        gt_pos_mask = explain.attribute(data, target=gt_cls)
# =============================================================================
#     #gt_pos_mask = np.sum(detach(gt_pos_mask).squeeze(), axis=0)
#     sorted_pred = torch.argsort(ori_pred[ori_pred!= gt_cls], descending=True)
#     sorted_pred_wo_gt = sorted_pred[sorted_pred!= gt_cls]
...
print("Explainablit Methods: ", EM)
print("Smooth Grad: ", smooth_grad)
...
# =============================================================================
    ...
    gt_mask = IG_purify_top_k(input_tensor, classifier, ori_cls, pred, n_csder=2, smooth=smooth_grad)
